...
namespace
{
  ...
  auto compile(
      ...
      unsigned const N,
      hs_platform_info_t const *const platform) -> std::pair<char *, std::size_t>
  {
    hs_database_t *db;
  ...
  auto l_serialize_regexp_db(lua_State *const L) -> int
  {
    ...
    // INDEX 3 - anchor for the strings we're processing
    // the strings we get out of the first argument might
    // be generated by a metatable and not actually anchored
    // anywhere
    ...
    for (lua_Integer i = 1; i <= N; i++)
    {
      ...
      flags[i - 1] = is_num ? flag : default_flags;
      ...
      lua_rawseti(L, 3, i); // save the string in a temporary table
      lua_pop(L, 3); // drops the table, flag, id

      exprs[i - 1] = str;
    }

    auto const [serialize_bytes, serialize_len] = compile(L, exprs, flags, ids, N, has_platform ? &platform : nullptr);
