...
{
    // this takes ownership of the addrinfo list and will deallocate it
...
{
    auto const app = App::from_loop(req->loop);
    auto const L = app->L;
    auto const loop = &app->loop;
...
{
    auto const loop = &App::from_lua(L)->loop;

    char const* hostname = luaL_checkstring(L, 1);
    luaL_checkany(L, 2);
    lua_settop(L, 2);

    auto const hints = addrinfo{ .ai_socktype = SOCK_STREAM };

    auto req = std::make_unique<uv_getaddrinfo_t>();
    uvok(uv_getaddrinfo(loop, req.get(), on_dnslookup, hostname, nullptr, &hints));
